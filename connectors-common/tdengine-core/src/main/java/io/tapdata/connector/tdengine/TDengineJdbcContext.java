...
public class TDengineJdbcContext extends JdbcContext {
	...
	public TDengineJdbcContext(TDengineConfig config, HikariDataSource hikariDataSource) {
...
//			query(String.format(PG_ALL_TABLE, getConfig().getDatabase(), getConfig().getSchema(), tableSql),
			query(String.format("select table_name, table_comment, type from information_schema.ins_tables where db_name = '%s' %s;", getConfig().getSchema(), tableSql),
...
//		String tableSql = EmptyKit.isNotEmpty(tableNames) ? "AND table_name IN (" + StringKit.joinString(tableNames, "'", ",") + ")" : "";
		try {
			for (String tableName : tableNames) {
				String sql = String.format("DESCRIBE %s.%s", getConfig().getSchema(), tableName);
				query(sql, resultSet -> {
...
//					resultSet -> columnList.addAll(DbKit.getDataFromResultSet(resultSet)));
	...
	public List<DataMap> queryAllIndexes(List<String> tableNames) {
		...
		try {
			query(String.format(PG_ALL_INDEX, getConfig().getDatabase(), getConfig().getSchema(), tableSql),
					resultSet -> indexList.addAll(TDengineDbKit.getDataFromResultSet(resultSet)));
		} catch (Throwable e) {
	...
	public boolean tableExists(String tableName) throws Throwable {
		...
		String sql = String.format("SELECT * FROM information_schema.tables WHERE TABLE_SCHEMA='%s' AND TABLE_NAME='%s'", getConfig().getSchema(), tableName);

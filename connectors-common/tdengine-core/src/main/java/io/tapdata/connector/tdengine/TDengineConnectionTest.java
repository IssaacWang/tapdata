...
import io.tapdata.connector.tdengine.constant.TDengineTestItem;
...
public class TDengineConnectionTest {
	...
	private TDengineJdbcContext mysqlJdbcContext;

	public TDengineConnectionTest(TDengineJdbcContext mysqlJdbcContext) {
	...
	public TestItem testHostPort(TapConnectionContext tapConnectionContext) {
		...
		try {
			...
			return testItem(TDengineTestItem.HOST_PORT.getContent(), TestItem.RESULT_SUCCESSFULLY);
		} catch (IOException e) {
			return testItem(TDengineTestItem.HOST_PORT.getContent(), TestItem.RESULT_FAILED, e.getMessage());
		}
	}
...
//					if (diff < 0) {
//						return testItem(TDengineTestItem.CHECK_VERSION.getContent(), TestItem.RESULT_FAILED, "Unsupported this MYSQL database version: " + version);
//					}
//				} else {
//					return testItem(TDengineTestItem.CHECK_VERSION.getContent(), TestItem.RESULT_FAILED, "Unsupported this MYSQL database version: " + version);
//				}
//			}
			...
			return testItem(TDengineTestItem.CHECK_VERSION.getContent(), TestItem.RESULT_FAILED, "Error checking version, reason: " + e.getMessage());
		}
		return testItem(TDengineTestItem.CHECK_VERSION.getContent(), TestItem.RESULT_SUCCESSFULLY);
	}
		...
		try {
						...
						for (String privilege : split) {
							...
							if (match) {
								if (cdcPrivilege.onlyNeed) {
									testItem.set(testItem(TDengineTestItem.CHECK_CDC_PRIVILEGES.getContent(), TestItem.RESULT_SUCCESSFULLY));
									return;
								}
								break;
							}
			...
			if (null == testItem.get()) {
				if (CollectionUtils.isNotEmpty(cdcPrivileges) && cdcPrivileges.size() > 1) {
					...
					testItem.set(testItem(TDengineTestItem.CHECK_CDC_PRIVILEGES.getContent(), TestItem.RESULT_SUCCESSFULLY_WITH_WARN,
			...
			if (errorCode == 1290 && "HY000".equals(sqlState) && StringUtils.isNotBlank(message) && message.contains("--skip-grant-tables")) {
				return testItem(TDengineTestItem.CHECK_CDC_PRIVILEGES.getContent(), TestItem.RESULT_SUCCESSFULLY);
			} else {
				return testItem(TDengineTestItem.CHECK_CDC_PRIVILEGES.getContent(), TestItem.RESULT_SUCCESSFULLY_WITH_WARN,
		...
		try {
			mysqlJdbcContext.query(CHECK_DATABASE_BINLOG_STATUS_SQL, resultSet -> {
				...
				if (!"ROW".equalsIgnoreCase(mode) || !"ON".equalsIgnoreCase(logbin)) {
					testItem.set(testItem(TDengineTestItem.CHECK_BINLOG_MODE.getContent(), TestItem.RESULT_SUCCESSFULLY_WITH_WARN,
							"MySqlServer dose not open row level binlog mode, will not be able to use the incremental sync feature"));
				} else {
					testItem.set(testItem(TDengineTestItem.CHECK_BINLOG_MODE.getContent(), TestItem.RESULT_SUCCESSFULLY));
				}
			});
		} catch (SQLException e) {
			return testItem(TDengineTestItem.CHECK_BINLOG_MODE.getContent(), TestItem.RESULT_SUCCESSFULLY_WITH_WARN,
			...
			mysqlJdbcContext.query(CHECK_DATABASE_BINLOG_ROW_IMAGE_SQL, resultSet -> {
				while (resultSet.next()) {
					...
					if (!StringUtils.equalsAnyIgnoreCase("FULL", value)) {
						testItem.set(testItem(TDengineTestItem.CHECK_BINLOG_ROW_IMAGE.getContent(), TestItem.RESULT_SUCCESSFULLY_WITH_WARN,
								"binlog row image is [" + value + "]"));
					}
				}
			});
			if (null == testItem.get()) {
				testItem.set(testItem(TDengineTestItem.CHECK_BINLOG_ROW_IMAGE.getContent(), TestItem.RESULT_SUCCESSFULLY));
			}
		} catch (Throwable e) {
			return testItem(TDengineTestItem.CHECK_BINLOG_ROW_IMAGE.getContent(), TestItem.RESULT_SUCCESSFULLY_WITH_WARN,
	...
	public TestItem testCreateTablePrivilege(TapConnectionContext tapConnectionContext) {
		try {
			...
			if (missed) {
				return testItem(TDengineTestItem.CHECK_CREATE_TABLE_PRIVILEGE.getContent(), TestItem.RESULT_SUCCESSFULLY_WITH_WARN,
						"User does not have privileges [ create ], will not be able to use the create table(s) feature");
			}
			return testItem(TDengineTestItem.CHECK_CREATE_TABLE_PRIVILEGE.getContent(), TestItem.RESULT_SUCCESSFULLY);
		} catch (SQLException e) {
			int errorCode = e.getErrorCode();
			String sqlState = e.getSQLState();
			String message = e.getMessage();
